...
gitplex.commons = {
	...
	form: {
		...
		markDirty: function($forms) {
			$forms.addClass("dirty").each(function() {
				gitplex.commons.form.dirtyChanged($(this));
			});
		},
		...
		removeDirty: function(triggerId, $forms) {
			$(function() {
				...
				$trigger.click(function(event){
					gitplex.commons.form.markClean($forms);
					previousClick(event);
				});
			});
		},
		trackDirty: function(form) {
			...
			if ($form.find(".dirty-aware").length != 0 || $form.hasClass("leave-confirm")) {
				...
				gitplex.commons.form.dirtyChanged($form);
			}
		},
		...
		setupDirtyCheck: function() {
			$("form").each(function() {
				gitplex.commons.form.trackDirty(this);
			});
	...
	focus: {
		...
		focusOn: function(componentId) {
			if (componentId)
				gitplex.commons.focus.doFocus($("#" + componentId));
			gitplex.commons.focus.$components = null;
		},
		...
		setupAutoFocus: function() {
			if (typeof(Wicket) != "undefined" && typeof(Wicket.Focus) != "undefined") {
				...
				Wicket.Focus.setFocusOnId = function(componentId) {
					gitplex.commons.focus.focusOn(componentId);
					wicketSetFocusOnId(componentId);
				}
			}
			
			Wicket.Event.subscribe('/ajax/call/beforeSend', function() {
				gitplex.commons.focus.$components = $();
			});
			Wicket.Event.subscribe('/ajax/call/complete', function() {
				if (gitplex.commons.focus.$components != null)
					gitplex.commons.focus.doFocus(gitplex.commons.focus.$components);
			});

			gitplex.commons.focus.doFocus($(document));

			$(document).on("elementReplaced", function(event, componentId) {
				if (gitplex.commons.focus.$components != null)
					gitplex.commons.focus.$components = gitplex.commons.focus.$components.add("#" + componentId);
			});			
		},
	...
	history: {
		...
		init: function(callback) {
			...
			setTimeout(function() {
				window.onpopstate = function(event) {
					if (gitplex.commons.history.getHashlessUrl(window.location.href) 
							!= gitplex.commons.history.getHashlessUrl(gitplex.commons.history.current.url)) {
						if (gitplex.commons.form.confirmLeave()) {
							if (!event.state || !event.state.data) {
								location.reload();
							} else {
								callback(event.state.data);
								gitplex.commons.history.current = {
									state: event.state,
							 ...
							 */
							history.pushState(gitplex.commons.history.current.state, '' , 
									gitplex.commons.history.current.url);
						}
					}
				};
			}, 100);
		...
		pushState: function(url, data) {
			...
			var state = {data: data};
			gitplex.commons.history.current = {state: state, url: url};
			history.pushState(state, '', url);
			setTimeout(function() {
				gitplex.commons.history.setVisited();
			}, 100);
		},
		/*
...
$(function() {
	gitplex.commons.setupAutoSize();
	gitplex.commons.setupAjaxLoadingIndicator();
	gitplex.commons.form.setupDirtyCheck();
	gitplex.commons.focus.setupAutoFocus();
	gitplex.commons.websocket.setupCallback();
});
