...
import com.turbodev.utils.RangeUtils;
...
import com.turbodev.server.util.diff.LineDiff;
...
import com.turbodev.server.web.component.diff.blob.SourceAware;
import com.turbodev.server.web.component.diff.blob.text.MarkAwareDiffBlock.Type;
import com.turbodev.server.web.component.diff.diffstat.DiffStatBar;
import com.turbodev.server.web.component.diff.difftitle.BlobDiffTitle;
import com.turbodev.server.web.component.diff.revision.BlobCommentSupport;
...
public class TextDiffPanel extends Panel implements SourceAware {
	...
	private String getJson(MarkPos mark) {
		try {
			...
			return TurboDev.getInstance(ObjectMapper.class).writeValueAsString(markInfo);
		...
		add(callbackBehavior = new AbstractPostAjaxBehavior() {
			...
			protected void respond(AjaxRequestTarget target) {
				...
				case "expand":
					...
					String script = String.format("turbodev.server.textDiff.expand('%s', %d, \"%s\");",
				...
				case "openSelectionPopover":
					...
					script = String.format("turbodev.server.textDiff.openSelectionPopover('%s', %s, %s, '%s', '%s', %s);", 
				...
				case "addComment":
					...
					script = String.format("turbodev.server.textDiff.onAddComment($('#%s'), %s);", 
				...
				case "toggleComment": 
					...
					if (!comment.equals(prevComment)) {
						script = String.format("turbodev.server.textDiff.onOpenComment($('#%s'), %s);", 
	...
	public void renderHead(IHeaderResponse response) {
		...
		if (commentSupport != null) {
			...
			ObjectMapper mapper = TurboDev.getInstance(ObjectMapper.class);
			try {
		...
		String script = String.format("turbodev.server.textDiff.onDomReady('%s', '%s', '%s', '%s', %s, %s, %s, '%s', %s,"
				...
				TurboDev.getInstance().getDocLink());
		
		response.render(OnDomReadyHeaderItem.forScript(script));

		script = String.format("turbodev.server.textDiff.onWindowLoad('%s', %b, %s);", 
	...
	public void onCommentDeleted(AjaxRequestTarget target, CodeComment comment) {
		String script = String.format("turbodev.server.textDiff.onCommentDeleted($('#%s'), %s);", 
				getMarkupId(), getJsonOfComment(comment));
		target.appendJavaScript(script);
		mark(target, null);
	}
	...
	public void onCommentClosed(AjaxRequestTarget target, CodeComment comment) {
		String script = String.format("turbodev.server.textDiff.onCloseComment($('#%s'));", getMarkupId());
		target.appendJavaScript(script);
		mark(target, null);
	}
	...
	public void onCommentAdded(AjaxRequestTarget target, CodeComment comment) {
		String script = String.format("turbodev.server.textDiff.onCommentAdded($('#%s'), %s);", 
				getMarkupId(), getJsonOfComment(comment));
		target.appendJavaScript(script);
	}
	...
	public void mark(AjaxRequestTarget target, MarkPos mark) {
		...
		if (mark != null) {
			script = String.format(""
				+ "var $container = $('#%s');"
				+ "var mark = %s;"
				+ "turbodev.server.textDiff.scrollTo($container, mark);"
				+ "turbodev.server.textDiff.mark($container, mark);", 
				getMarkupId(), getJson(mark));
		} else {
			script = String.format(""
				+ "var $container = $('#%s');"
				+ "turbodev.server.textDiff.clearMark($container);"
