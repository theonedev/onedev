...
import com.turbodev.server.web.page.project.commit.CommitQueryLexer;
import com.turbodev.server.web.page.project.commit.CommitQueryParser;
import com.turbodev.server.web.page.project.commit.CommitQueryParser.CriteriaContext;
import com.turbodev.server.web.page.project.commit.CommitQueryParser.FuzzyCriteriaContext;
import com.turbodev.server.web.page.project.commit.CommitQueryParser.QueryContext;
...
import com.turbodev.server.web.util.ajaxlistener.IndicateLoadingListener;
...
public class ProjectCommitsPage extends ProjectPage {
		...
		if (commit != null) {
			...
			item.add(new VerificationStatusPanel("verificationStatus", new LoadableDetachableModel<Map<String, Verification>>() {
				...
				protected Map<String, Verification> load() {
					return TurboDev.getInstance(VerificationManager.class).getVerifications(getProject(), commit.name());
				}
	...
	private String renderCommitGraph() {
		...
		return String.format("turbodev.server.commitgraph.render('%s', %s);", body.getMarkupId(), jsonOfCommits);
	}
