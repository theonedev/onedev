turbodev.server.textDiff = {
	...
	onDomReady: function(containerId, symbolTooltipId, oldRev, newRev, callback, blameMessageCallback,
		...
		$container.data("onMouseOverContent", function() {
			if (!turbodev.server.mouseState.pressed) {
		...
		var $symbols = $container.find(turbodev.server.textDiff.symbolClasses); 
		$symbols.mouseover($container.data("symbolHover"));
		$container.find("td.content").mouseover($container.data("onMouseOverContent"));

		turbodev.server.textDiff.initBlameTooltip(containerId, $container.find("td.blame>a.hash"));
		...
		$container.selectionPopover("init", function() {
			...
			function showInvalidSelection() {
				...
				$content.children("a").attr("href", doclink + "/diff-selection.md");
				return {
					position: position, 
					content: $content
				}
		...
		for (var line in oldComments) {
		    if (oldComments.hasOwnProperty(line)) {
		    	turbodev.server.textDiff.addCommentIndicator($container, true, line, oldComments[line][1]);
		    }
		}
		for (var line in newComments) {
		    if (newComments.hasOwnProperty(line)) {
		    	turbodev.server.textDiff.addCommentIndicator($container, false, line, newComments[line][1]);
		    }
		}

		turbodev.server.textDiff.highlightCommentTrigger($container);				
	...
	onWindowLoad: function(containerId, ajax, mark) {
		var $container = $("#" + containerId);
		if (!ajax && mark && !turbodev.server.history.isVisited()) {
	...
	expand: function(containerId, blockIndex, expandedHtml) {
		...
		var $symbols = $expandedTrs.find(turbodev.server.textDiff.symbolClasses); 
		$symbols.mouseover($container.data("symbolHover"));
		...
		turbodev.server.textDiff.initBlameTooltip(containerId, $expandedTrs.find(">td.blame>a.hash"));
	...
	scrollTo: function($container, mark) {
		var markInfo = turbodev.server.textDiff.getMarkInfo($container, mark);
	...
	clearMark: function($container) {
		$container.find("td.content.content-mark").each(function() {
			...
			$this.find(turbodev.server.textDiff.symbolClasses).mouseover($container.data("symbolHover"));
		});
	},
	...
	addCommentIndicator: function($container, leftSide, line, comments) {
		...
		var $lineNumTd = turbodev.server.textDiff.getLineNumTd($container, leftSide, line);
			...
			$indicator.on('shown.bs.popover', function () {
				$("." + oldOrNew + ".comment-popover[data-line='" + line + "'] a").each(function() {
					$(this).mouseover(function() {
						...
						turbodev.server.textDiff.mark($container, comment.mark);
					});
					$(this).mouseout(function() {
						turbodev.server.textDiff.restoreMark($container);
					});
					$(this).click(function() {
    					if ($("#"+$container.data("dirtyContainerId")).find("form.dirty").length != 0 
    							&& !confirm("There are unsaved changes, discard and continue?")) {
	...
	highlightCommentTrigger: function($container) {
		...
		if (openComment) {
			...
			var $indicator = turbodev.server.textDiff.getLineNumTd($container, openComment.mark.leftSide, line).children(".comment-indicator");
	...
	onCommentAdded: function($container, comment) {
		...
		var $indicator = turbodev.server.textDiff.getLineNumTd($container, leftSide, line).children(".comment-indicator");
		var comments;
		if ($indicator.length != 0) {
		...
		turbodev.server.textDiff.addCommentIndicator($container, leftSide, line, comments);
		turbodev.server.textDiff.highlightCommentTrigger($container);				
	},
	...
	onOpenComment: function($container, comment) {
		...
		turbodev.server.textDiff.highlightCommentTrigger($container);
		turbodev.server.textDiff.mark($container, comment.mark);
		turbodev.server.textDiff.scrollTo($container, comment.mark);
	},
