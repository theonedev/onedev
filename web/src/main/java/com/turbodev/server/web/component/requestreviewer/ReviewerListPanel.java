...
import com.turbodev.server.web.behavior.dropdown.DropdownHover;
			...
			protected void populateItem(ListItem<User> item) {
					...
					public void onClick(AjaxRequestTarget target) {
						...
						if (invitation != null) {
							...
							if (request.isNew()) {
								...
								removed = TurboDev.getInstance(ReviewInvitationManager.class).exclude(invitation);
							}
							if (!removed) {
				...
				item.add(new AjaxLink<Void>("withdraw") {
					...
					public void onClick(AjaxRequestTarget target) {
						...
						User reviewer = item.getModelObject();
						request.clearQualityStatus();
						TurboDev.getInstance(ReviewManager.class).delete(reviewer, request);
						reviewersModel.detach();
						send(getPage(), Broadcast.BREADTH, new PageDataChanged(target));								
					}
