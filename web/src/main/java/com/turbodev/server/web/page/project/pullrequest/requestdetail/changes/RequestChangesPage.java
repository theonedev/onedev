package com.turbodev.server.web.page.project.pullrequest.requestdetail.changes;
...
public class RequestChangesPage extends RequestDetailPage implements CommentSupport {
	...
	public RequestChangesPage(PageParameters params) {
		...
		if (state.commentId != null) {
			CodeComment comment = TurboDev.getInstance(CodeCommentManager.class).load(state.commentId);
			...
			protected Component newContent(String id, FloatingPanel dropdown) {
				...
				Fragment fragment = new Fragment(id, "commitsFrag", RequestChangesPage.this) {
					...
					public void renderHead(IHeaderResponse response) {
						...
						if ((state.oldCommit.equals(getPullRequest().getBaseCommitHash()) || oldIndex != -1) 
								&& newIndex != -1) {
							script = String.format("turbodev.server.requestChanges.initCommitSelector(%s, '%s', %d, %d);", 
									callback, getPullRequest().getBaseCommitHash(), oldIndex+1, newIndex);
						} else {
							script = String.format("turbodev.server.requestChanges.initCommitSelector(%s, '%s');", 
