turbodev.server.symboltooltip = {
	init: function(containerId, queryCallback, ajaxIndicatorUrl) {
		...
		var prepareToHide = function() {
			if (container.tooltip) {
				...
				container.tooltip.hideTimer = setTimeout(function(){
					turbodev.server.symboltooltip.removeTooltip(container);
				}, 200);
			}
		};
		...
		container.onMouseOverSymbol = function(revision, symbolEl) {
			var $symbol = $(symbolEl);
			if (turbodev.server.mouseState.pressed || !turbodev.server.mouseState.moved || $symbol.text() == "@")
				return;
...
$(function() {
	$(window).scroll(function() {
		$(".symbol-tooltip-container").each(function() {
			turbodev.server.symboltooltip.removeTooltip(this);
		});
	});
});