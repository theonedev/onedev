package com.turbodev.server.web.page.project.pullrequest.requestlist;
	...
	protected void onInitialize() {
		...
		IDataProvider<PullRequest> dataProvider = new IDataProvider<PullRequest>() {
			...
			public Iterator<? extends PullRequest> iterator(long first, long count) {
				...
				return TurboDev.getInstance(Dao.class).findRange(criteria, (int)first, (int)count).iterator();
			}

			@Override
			public long size() {
				ProjectPage page = (ProjectPage) getPage();
				return TurboDev.getInstance(Dao.class).count(searchOption.getCriteria(page.getProject()));
			}
			...
			public IModel<PullRequest> model(PullRequest object) {
				...
				return new LoadableDetachableModel<PullRequest>() {
					...
					protected PullRequest load() {
						return TurboDev.getInstance(Dao.class).load(PullRequest.class, pullRequestId);
					}
