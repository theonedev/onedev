...
public class SourceEditPanel extends BlobEditPanel implements Markable {
	...
	protected WebMarkupContainer newEditOptions(String componentId) {
		sourceFormat = new SourceFormatPanel(componentId, new OptionChangeCallback() {
			...
			public void onOptioneChange(AjaxRequestTarget target) {
				String script = String.format("turbodev.server.sourceEdit.onIndentTypeChange('%s', '%s');", 
			...
			@Override
			public void onOptioneChange(AjaxRequestTarget target) {
				String script = String.format("turbodev.server.sourceEdit.onTabSizeChange('%s', %s);", 
			...
			@Override
			public void onOptioneChange(AjaxRequestTarget target) {
				String script = String.format("turbodev.server.sourceEdit.onLineWrapModeChange('%s', '%s');", 
			...
			protected void updateAjaxAttributes(AjaxRequestAttributes attributes) {
				...
				attributes.getAjaxCallListeners().add(new IAjaxCallListener() {
					...
					public CharSequence getBeforeHandler(Component component) {
						return String.format("turbodev.server.sourceEdit.onSubmit('%s');", 
	...
	public void renderHead(IHeaderResponse response) {
		...
		String script = String.format("turbodev.server.sourceEdit.onDomReady('%s', '%s', %s, '%s', %s, '%s', %b, '%s');", 
				sourceFormComponent.getMarkupId(), 
		...
		script = String.format("turbodev.server.sourceEdit.onWindowLoad('%s', %s, '%s');", 
	...
	public void mark(AjaxRequestTarget target, TextRange mark) {
		String script;
		if (mark != null) {
			script = String.format("turbodev.server.sourceEdit.mark('%s', %s);", 
					sourceFormComponent.getMarkupId(), getJson(mark));
		} else {
			script = String.format("turbodev.server.sourceEdit.mark('%s', undefined);", 
					sourceFormComponent.getMarkupId());
		}
		target.appendJavaScript(script);
	}
