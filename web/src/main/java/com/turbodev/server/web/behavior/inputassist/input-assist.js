turbodev.server.inputassist = {
	init: function(inputId, callback) {
		...
		$input.on("input click keyup", function(e) {
			...
			if (value.trim().length == 0)
				turbodev.server.inputassist.markErrors(inputId, []);
		});
		function onFocus() {
			var value = $input.val();
		...
		$input.bind("keydown", "up", function() {
			...
			if ($dropdown) {
				...
				turbodev.server.inputassist.updateHelp($dropdown);
	...
	assistOpened: function(inputId, dropdownId, inputContent) {
		...
		turbodev.server.inputassist.assistUpdated(inputId, dropdownId, inputContent);
	},
