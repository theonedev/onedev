...
public class UnexpectedExceptionPage extends BaseErrorPage {
	...
	protected void onInitialize() {
		...
		serverProps.add(new Property("TurboDev Version", "EAP"));
		User user = getLoginUser();
		if (user != null) 
			serverProps.add(new Property("TurboDev User", user.getName()));
		else 
			serverProps.add(new Property("TurboDev User", "<anonymous>"));
		...
		add(new WebMarkupContainer("backUrl") {
			...
			protected void onComponentTag(ComponentTag tag) {
				super.onComponentTag(tag);
				tag.put("value", TurboDev.getInstance(ConfigManager.class).getSystemSetting().getServerUrl());
			}
