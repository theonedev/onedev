...
import com.turbodev.server.web.page.project.blob.search.quick.QuickSearchPanel;
...
import com.turbodev.server.web.websocket.CodeCommentChangedRegion;
import com.turbodev.server.web.websocket.CommitIndexedRegion;
...
public class ProjectBlobPage extends ProjectPage implements BlobRenderContext {
		...
		add(revisionIndexing = new WebMarkupContainer("revisionIndexing") {
			...
			protected void onConfigure() {
				...
				if (!indexManager.isIndexed(getProject(), commit)) {
					TurboDev.getInstance(IndexManager.class).indexAsync(getProject(), commit);
					setVisible(true);
				} else {
					setVisible(false);
				}
	...
	public CodeComment getOpenComment() {
		if (state.commentId != null)
			return TurboDev.getInstance(CodeCommentManager.class).load(state.commentId);
		else
			return null;
	}
	...
	public PullRequest getPullRequest() {
		if (state.requestId != null)
			return TurboDev.getInstance(PullRequestManager.class).load(state.requestId);
		else
			return null;
	}
	...
	private void newBlobContent(@Nullable AjaxRequestTarget target) {
		...
		for (BlobRendererContribution contribution: TurboDev.getExtensions(BlobRendererContribution.class)) {
	...
	private void onResolvedRevisionChange(AjaxRequestTarget target) {
		...
		TurboDev.getInstance(WebSocketManager.class).onRegionChange(this);
	...
	public void renderHead(IHeaderResponse response) {
		...
		String script = String.format("turbodev.server.projectBlob.onDomReady(%s);", callback);
		
		response.render(OnDomReadyHeaderItem.forScript(script));
		
		response.render(OnLoadHeaderItem.forScript("turbodev.server.projectBlob.onWindowLoad();"));
	}
