package com.turbodev.server.web.component.floating;
...
public abstract class FloatingPanel extends Panel {
	...
	protected void onInitialize() {
		...
		add(new AbstractPostAjaxBehavior() {
			...
			public void renderHead(Component component, IHeaderResponse response) {
				...
				String script = String.format("turbodev.server.floating.init('%s', {target:%s, placement:%s}, %s);", 
	...
	public final void close() {
		...
		if (target != null) {
			String script = String.format("turbodev.server.floating.close($('#%s'), false);", getMarkupId(true));
