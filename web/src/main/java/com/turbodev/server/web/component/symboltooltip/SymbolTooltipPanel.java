package com.turbodev.server.web.component.symboltooltip;
...
import com.turbodev.server.search.IndexConstants;
import com.turbodev.server.search.SearchManager;
import com.turbodev.server.search.hit.QueryHit;
import com.turbodev.server.search.hit.SymbolHit;
	...
	protected void onInitialize() {
			...
			protected void populateItem(ListItem<QueryHit> item) {
				...
				AjaxLink<Void> link = new ViewStateAwareAjaxLink<Void>("link") {
					...
					public void onClick(AjaxRequestTarget target) {
						String script = String.format("turbodev.server.symboltooltip.removeTooltip(document.getElementById('%s'));", 
		...
		add(new AbstractPostAjaxBehavior() {
			...
			protected void respond(AjaxRequestTarget target) {
				...
				if (symbolName.length() != 0 && symbolName.indexOf('?') == -1 && symbolName.indexOf('*') == -1) {
					...
					if (symbolHits.size() < QUERY_ENTRIES) {
						...
						List<Symbol> symbols = TurboDev.getInstance(SearchManager.class).getSymbols(projectModel.getObject(), 
				...
				target.add(content);
				String script = String.format("turbodev.server.symboltooltip.doneQuery('%s');", content.getMarkupId());
				target.appendJavaScript(script);
			}
			...
			public void renderHead(Component component, IHeaderResponse response) {
				...
				String script = String.format("turbodev.server.symboltooltip.init('%s', %s, '%s');", 
