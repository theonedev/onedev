turbodev.server.markdown = {
	...
	onDomReady: function(containerId, callback, atWhoLimit, attachmentSupport, 
		...
		$editLink.click(function() {
			...
			onLayoutChange();
			Cookies.set(turbodev.server.markdown.getCookiePrefix($container)+".split", false, {expires: Infinity});
		});
		...
		$splitLink.click(function() {
			...
			onLayoutChange();
			callback("render", $input.val());
			Cookies.set(turbodev.server.markdown.getCookiePrefix($container)+".split", true, {expires: Infinity});
		});
		...
		$input.doneEvents("keydown", function(e) {
			if (e.keyCode>=33 && e.keyCode<=40 && $preview.is(":visible")) {
				...
				turbodev.server.markdown.syncPreviewScroll(containerId);
			}
		}, 500);
		...
		$input.on("keydown", function(e) {
			if (e.keyCode == 13) {
				...
				turbodev.server.markdown.fireInputEvent($input);
			}
		});
		...
		$edit.resizable({
			...
			stop: function(e, ui) {
				Cookies.set(turbodev.server.markdown.getCookiePrefix($container)+".inputHeight", 
						$input.outerHeight(), {expires: Infinity});
				if ($container.hasClass("normal-mode") && $container.hasClass("split-mode")) {
					Cookies.set(turbodev.server.markdown.getCookiePrefix($container)+".renderedHeight", 
		...
		$warning.on("closed.bs.alert", function () {
			if (!$resizeHandles.is(":visible"))
				turbodev.server.markdown.autofit(containerId, $container.outerWidth(), $container.outerHeight());
		})
		
		$container.on("autofit", function(e, width, height) {
			$resizeHandles.hide();
			turbodev.server.markdown.autofit(containerId, width, height);
		});
		...
		$head.find(".do-fullscreen").click(function() {
			...
			if ($container.hasClass("fullscreen")) {
				...
				if ($resizeHandles.is(":visible")) {
					...
					turbodev.server.markdown.restoreSizeFromCookie($container);
				} else {
					$(window).resize();
				}
			} else {
		...
		$(window).resize(function() {
			if ($container.hasClass("fullscreen")) {
				turbodev.server.markdown.autofit(containerId, $(window).width(), $(window).height());
			}
		});
	    ...
	    if (attachmentSupport) {
			...
			function uploadFile(file) {
				if (file.size> attachmentMaxSize) {
					...
					turbodev.server.markdown.updateUploadMessage($input, message);
				} else {
					...
					xhr.onload = function() {
						if (xhr.status == 200) { 
							...
							turbodev.server.markdown.updateUploadMessage($input, 
    ...
    restoreSizeFromCookie: function($container) {
		...
		var inputHeight = Cookies.get(turbodev.server.markdown.getCookiePrefix($container)+".inputHeight");
		if (inputHeight) {
		...
		$edit.outerHeight($input.outerHeight());
		var renderedHeight = Cookies.get(turbodev.server.markdown.getCookiePrefix($container)+".renderedHeight");
		if (renderedHeight) {
	...
	onWindowLoad: function(containerId) {
		...
		if (autosaveKey) {
			turbodev.server.form.registerAutosaveKey($container.closest("form.leave-confirm"), autosaveKey);
	...
	onRendered: function(containerId, html) {
		...
		$rendered.html(html);
		turbodev.server.markdown.initRendered($rendered);
	...
	initRendered: function($rendered) {
		turbodev.server.highlight($rendered);
	...
	onViewerDomReady: function(containerId, taskCallback, taskSourcePositionDataAttribute) {
		...
		turbodev.server.markdown.initRendered($container.find(".markdown-rendered"));
	},
	...
	insertUrl: function(containerId, isImage, url, name, replaceMessage) {
    	...
    	turbodev.server.markdown.updateUploadMessage($input, message, replaceMessage);
