package io.onedev.server.util.inputspec.teamchoiceinput;
...
import io.onedev.server.util.inputspec.teamchoiceinput.choiceprovider.AllTeams;
import io.onedev.server.util.inputspec.teamchoiceinput.choiceprovider.ChoiceProvider;
import io.onedev.server.util.inputspec.teamchoiceinput.defaultvalueprovider.DefaultValueProvider;
import io.onedev.server.util.inputspec.teamchoiceinput.defaultvalueprovider.SpecifiedDefaultValue;
import io.onedev.server.web.editable.annotation.Editable;
import io.onedev.server.web.editable.annotation.NameOfEmptyValue;
import jersey.repackaged.com.google.common.collect.Lists;

@Editable(order=160, name=InputSpec.TEAM)
public class TeamChoiceInput extends InputSpec {
	
	private static final long serialVersionUID = 1L;

	private ChoiceProvider choiceProvider = new AllTeams();
	...
	public List<String> getPossibleValues() {
		...
		if (OneDev.getInstance(Validator.class).validate(getChoiceProvider()).isEmpty()) {
			for (TeamFacade team: getChoiceProvider().getChoices(true))
				possibleValues.add(team.getName());
		}
	...
	public String getPropertyDef(Map<String, Integer> indexes) {
		...
		appendChoiceProvider(buffer, index, "@TeamChoice");
