	...
	contribs: {
		onDomReady : function(overallContributions, topContributorsDataUrl) {
			...
			function updateDateRange() {
				$contribs.find(".date-range").text(formatDate(fromDay) + " ~ " + formatDate(toDay));
			}
			...
			for (var i=0; i<days.length; i++) {
				overallXAxisData.push(formatDate(days[i]));
			}
			...
			function formatYAxisLabel(value, index, useKilo) {
    			if (index == 0)
    				useKilo = false;
				...
				yAxis: {
			    	...
			    	axisLabel: {
			    		formatter: function(value, index) {
			    			var result = formatYAxisLabel(value, index, useKiloForOverallChart);
						...
						success: function(topContributors) {
							...
							for (var i=0; i<days.length; i++) {
								if (days[i] >= fromDay && days[i] <= toDay)
									xAxisData.push(formatDate(days[i]));
							}
							...
							function renderContributor($container, contributor, index) {
								...
								var option = {
									...
									yAxis: {
								    	...
								    	axisLabel: {
								    		formatter: function(value, index) {
								    			var result = formatYAxisLabel(value, index, useKilo);
