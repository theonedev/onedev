...
public abstract class WorkflowReconcilePanel extends Panel {
	...
	private Component checkFields(String markupId) {
		return new AjaxLazyLoadPanel(markupId) {
			...
			@Override
			public Component getLazyLoadComponent(String markupId) {
				Map<String, String> invalidFieldMap = getIssueFieldManager().getInvalidFields(getProject());
				if (!invalidFieldMap.isEmpty()) {
					...
					form.add(new AjaxButton("fix") {
						...
						protected void onSubmit(AjaxRequestTarget target, Form<?> form) {
							...
							getIssueFieldManager().fixInvalidFields(getProject(), resolutions);
	...
	private Component checkFieldValues(String markupId) {
		return new AjaxLazyLoadPanel(markupId) {
			...
			@Override
			public Component getLazyLoadComponent(String markupId) {
				Map<String, String> undefinedFieldValues = getIssueFieldManager().getUndefinedFieldValues(getProject());
				if (!undefinedFieldValues.isEmpty()) {
					...
					form.add(new AjaxButton("fix") {
						...
						protected void onSubmit(AjaxRequestTarget target, Form<?> form) {
							super.onSubmit(target, form);
							getIssueFieldManager().fixUndefinedFieldValues(getProject(), resolutions);
	...
	private Component checkFieldValueOrders(String markupId) {
		return new AjaxLazyLoadPanel(markupId) {
			...
			@Override
			public Component getLazyLoadComponent(String markupId) {
				getIssueFieldManager().fixFieldValueOrders(getProject());
