...
import io.onedev.server.ee.xsearch.hit.TextHit;
	...
	public void collect(Document document, List<QueryHit> hits) {
		...
		try (var objectReader = repository.newObjectReader()) {
			try {
				...
				for (var match: getOption().matches(blobPath, objectLoader, getCount())) 
					hits.add(new TextHit(projectId, blobPath, match.getPosition(), match.getLine()));
			} catch (IOException e) {
