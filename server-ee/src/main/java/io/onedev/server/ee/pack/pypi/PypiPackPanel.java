...
import static io.onedev.server.ee.pack.pypi.PypiPackService.SERVICE_ID;
...
public class PypiPackPanel extends GenericPanel<Pack> {
	...
	protected void onInitialize() {
		...
		var indexUrl = protocol + "://<OneDev_account_name>:<OneDev_password>@" + UrlUtils.getServer(serverUrl) 
				+ "/" + getPack().getProject().getPath() + "/~" + SERVICE_ID + "/simple/";
		var installCmd = "$ python3 -m pip install --index-url " + indexUrl;
