...
public class DashboardPage extends LayoutPage {
	...
	private static final int HORIZONTAL_CELL_COUNT = 48;
	...
	private void initWidgetPosition(Widget widget) {
		int top = 0;
		while (true) {
			for (int left = 0; left <= HORIZONTAL_CELL_COUNT - widget.getDefaultWidth(); left++) {
				widget.setLeft(left);
				widget.setTop(top);
				widget.setRight(left + widget.getDefaultWidth());
