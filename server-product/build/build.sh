...
docker run -it --rm -v /var/run/docker.sock:/var/run/docker.sock -v \$(which docker):/usr/bin/docker -v \$(pwd)/onedev:/opt/onedev -p 6610:6610 1dev/server:${buildVersion}
EOF
chmod +x ../target/docker/run.sh

unzip ../target/onedev-${buildVersion}.zip -d ../target/docker
mv ../target/docker/onedev-${buildVersion} ../target/docker/app
cp ../target/sandbox/site/lib/mysql* ../target/sandbox/site/lib/postgresql* ../target/docker/app/site/lib
