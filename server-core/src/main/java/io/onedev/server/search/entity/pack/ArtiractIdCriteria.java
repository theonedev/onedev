...
public class ArtiractIdCriteria extends Criteria<Pack> {
	...
	public ArtiractIdCriteria(String value, int operator) {
		this.value = value;
	...
	public Predicate getPredicate(CriteriaQuery<?> query, From<Pack, Pack> from, CriteriaBuilder builder) {
		Path<String> attribute = from.get(Pack.PROP_ARTIFACT_ID);
	...
	public boolean matches(Pack pack) {
		String artifactId = pack.getArtifactId();
		var matches = artifactId != null && matchString(value.toLowerCase(), artifactId.toLowerCase());
		if (operator == PackQueryLexer.IsNot)
			matches = !matches;
		return matches;
	}
	...
	public String toStringWithoutParens() {
		return quote(Pack.NAME_ARTIFACT_ID) + " " 
