...
public class IssueEmptyCriteria extends Criteria<Workspace> {

	private static final long serialVersionUID = 1L;

	private final int operator;

	public IssueEmptyCriteria(int operator) {
	...
	public Predicate getPredicate(@Nullable ProjectScope projectScope, CriteriaQuery<?> query,
		...
		var predicate = builder.isNull(from.get(Workspace.PROP_ISSUE));
	...
	public boolean matches(Workspace workspace) {
		var matches = workspace.getIssue() == null;
		if (operator == WorkspaceQueryLexer.IsNotEmpty)
			matches = !matches;
		return matches;
	}
	...
	public String toStringWithoutParens() {
		return quote(Workspace.NAME_ISSUE) + " " + WorkspaceQuery.getRuleName(operator);
	}
