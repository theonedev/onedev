...
public class LinkCriteria extends IssueCriteria {
	...
	public Predicate getPredicate(CriteriaQuery<?> query, Root<Issue> root, CriteriaBuilder builder) {
		...
		if (opposite) {
			...
			predicates.add(builder.equal(linkRoot.get(IssueLink.PROP_SOURCE), issue));
		} else if (linkSpec.getOpposite() != null) {
			predicates.add(builder.equal(linkRoot.get(IssueLink.PROP_SOURCE), root));
			predicates.add(builder.equal(linkRoot.get(IssueLink.PROP_TARGET), issue));
		} else {
			predicates.add(builder.or(
					builder.and(
							builder.equal(linkRoot.get(IssueLink.PROP_SOURCE), root), 
							builder.equal(linkRoot.get(IssueLink.PROP_TARGET), issue)),
					builder.and(
							builder.equal(linkRoot.get(IssueLink.PROP_SOURCE), issue), 
