...
class ConfigFileListEditPanel extends PropertyEditor<List<Serializable>> {

	private final List<ConfigFile> configFiles;
	
	public ConfigFileListEditPanel(String id, PropertyDescriptor propertyDescriptor, IModel<List<Serializable>> model) {
	...
	protected void onInitialize() {
		...
		add(new ModalLink("addNew") {
			...
			protected Component newContent(String id, ModalPanel modal) {
				return new ConfigFileEditPanel(id, configFiles, -1) {
					...
					protected void onSave(AjaxRequestTarget target) {
						...
						target.add(ConfigFileListEditPanel.this);
					}
		...
		columns.add(new AbstractColumn<ConfigFile, Void>(Model.of("")) {
			...
			@Override
			public void populateItem(Item<ICellPopulator<ConfigFile>> cellItem, String componentId, IModel<ConfigFile> rowModel) {
				Fragment fragment = new Fragment(componentId, "actionColumnFrag", ConfigFileListEditPanel.this);
				fragment.add(new ModalLink("edit") {

					@Override
					protected Component newContent(String id, ModalPanel modal) {
						return new ConfigFileEditPanel(id, configFiles, cellItem.findParent(Item.class).getIndex()) {
				...
				fragment.add(new AjaxLink<Void>("delete") {
					...
					public void onClick(AjaxRequestTarget target) {
						markFormDirty(target);
						configFiles.remove(rowModel.getObject());
						onPropertyUpdating(target);
						target.add(ConfigFileListEditPanel.this);
					}
		...
		DataTable<ConfigFile, Void> dataTable;
		add(dataTable = new DataTable<ConfigFile, Void>("configFiles", columns, dataProvider, Integer.MAX_VALUE));
		...
		dataTable.add(new SortBehavior() {
			...
			protected void onSort(AjaxRequestTarget target, SortPosition from, SortPosition to) {
				CollectionUtils.move(configFiles, from.getItemIndex(), to.getItemIndex());
				onPropertyUpdating(target);
				target.add(ConfigFileListEditPanel.this);
			}
