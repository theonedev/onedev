onedev.server.buildSpec = {
    onDomReady: function(selection, selectCallback, deleteCallback) {
        ...
        var $body = $(".build-spec>.valid>.jobs>.body");
        ...
        onedev.server.buildSpec.showJob(activeJobIndex);
        ...
        $body.find(".side>.navs>.nav>.select").mouseup(onedev.server.buildSpec.selectJob);
        $body.find(".side>.navs>.nav>.delete").mouseup(onedev.server.buildSpec.deleteJob);
    },
    selectJob: function() {
        var $nav = $(this).parent();
    ...
    deleteJob: function() {
        var $nav = $(this).parent();
        if (!$nav.hasClass("ui-sortable-helper")) {
            var $body = $(".build-spec-edit .jobs>.body");
            var $navs = $body.find(">.side>.navs");
            var $contents = $body.children(".contents");
            var index = $nav.index();
            var $nav = $navs.children().eq(index);
    ...
    swapJobs: function(index1, index2) {
        var $contents = $(".build-spec-edit .jobs>.body>.contents");
