package io.onedev.server.web.page.admin.clustermanagement;
...
public class ClusterManagementPage extends AdministrationPage {
	...
	protected void onInitialize() {
		...
		add(new ListView<String>("servers", new LoadableDetachableModel<>() {
			...
			protected void populateItem(ListItem<String> item) {
				var escaped = escapeHtml5(item.getModelObject());
				if (item.getIndex() == 0) 
	...
	protected Component newTopbarTitle(String componentId) {
		return new Label(componentId, "Clustering & Replication");
	}
