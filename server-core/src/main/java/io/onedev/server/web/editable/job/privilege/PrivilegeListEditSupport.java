...
public class PrivilegeListEditSupport implements EditSupport {
			...
			if (elementClass == JobPrivilege.class) {
				return new PropertyContext<List<Serializable>>(descriptor) {
						...
						return new PropertyViewer(componentId, descriptor) {
							...
							protected Component newContent(String componentId, PropertyDescriptor propertyDescriptor) {
								if (model.getObject() != null) 
									return new PrivilegeListViewPanel(componentId, model.getObject());
								else 
					...
					public PropertyEditor<List<Serializable>> renderForEdit(String componentId, IModel<List<Serializable>> model) {
						return new PrivilegeListEditPanel(componentId, descriptor, model);
					}
