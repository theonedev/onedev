...
class CacheConfigListViewPanel extends Panel {

	private final List<CacheConfig> configs = new ArrayList<>();
	
	public CacheConfigListViewPanel(String id, List<Serializable> elements) {
		super(id);
	...
	protected void onInitialize() {
		...
		columns.add(new AbstractColumn<CacheConfig, Void>(Model.of("")) {
			...
			public void populateItem(Item<ICellPopulator<CacheConfig>> cellItem, String componentId, IModel<CacheConfig> rowModel) {
				...
				Fragment fragment = new Fragment(componentId, "showDetailFrag", CacheConfigListViewPanel.this);
				fragment.add(new AjaxLink<Void>("link") {
					...
					public void onClick(AjaxRequestTarget target) {
						new DrawCardPanel(target, DrawPanel.Placement.RIGHT) {
							...
							protected void onInitialize() {
								super.onInitialize();
								add(AttributeAppender.append("class", "cache-config"));
							}
		...
		add(new DataTable<CacheConfig, Void>("configs", columns, dataProvider, Integer.MAX_VALUE) {
	...
	public void renderHead(IHeaderResponse response) {
		...
		response.render(CssHeaderItem.forReference(new CacheConfigCssResourceReference()));
	}
