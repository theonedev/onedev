...
public class CacheConfigListEditSupport implements EditSupport {
	...
	public PropertyContext<?> getEditContext(PropertyDescriptor descriptor) {
		if (List.class.isAssignableFrom(descriptor.getPropertyClass())) {
			...
			if (elementClass == CacheConfig.class) {
				return new PropertyContext<List<Serializable>>(descriptor) {
						...
						return new PropertyViewer(componentId, descriptor) {
							...
							protected Component newContent(String id, PropertyDescriptor propertyDescriptor) {
								if (model.getObject() != null) {
									return new CacheConfigListViewPanel(id, model.getObject());
								} else {
					...
					public PropertyEditor<List<Serializable>> renderForEdit(String componentId, IModel<List<Serializable>> model) {
						return new CacheConfigListEditPanel(componentId, descriptor, model);
					}
