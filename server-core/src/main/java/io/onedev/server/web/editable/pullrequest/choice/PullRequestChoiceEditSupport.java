...
public class PullRequestChoiceEditSupport implements EditSupport {
	...
	public PropertyContext<?> getEditContext(PropertyDescriptor descriptor) {
        ...
        if (pullRequestChoice != null) {
			var useNumber = pullRequestChoice.useNumber();
        	if (propertyGetter.getReturnType() == Long.class) {
        		return new PropertyContext<Long>(descriptor) {
					...
					public PropertyViewer renderForView(String componentId, final IModel<Long> model) {
						return new PropertyViewer(componentId, descriptor) {
							...
							protected Component newContent(String id, PropertyDescriptor propertyDescriptor) {
								Long pullRequestIdOrNumber = model.getObject();
								if (pullRequestIdOrNumber != null) {
									if (useNumber) {
										return new Label(id, "#" + String.valueOf(pullRequestIdOrNumber));
									} else {
										PullRequest request = OneDev.getInstance(PullRequestService.class).get(pullRequestIdOrNumber);
										if (request != null) {
					...
					public PropertyEditor<Long> renderForEdit(String componentId, IModel<Long> model) {
						return new PullRequestChoiceEditor(componentId, descriptor, model, useNumber);
					}
