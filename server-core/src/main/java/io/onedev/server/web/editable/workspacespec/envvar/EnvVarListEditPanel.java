...
class EnvVarListEditPanel extends DrawCardBeanListEditPanel<EnvVar> {

	private static final long serialVersionUID = 1L;

	EnvVarListEditPanel(String id, PropertyDescriptor propertyDescriptor, IModel<List<Serializable>> model) {
		super(id, propertyDescriptor, model);
	}

	@Override
	protected String getAddTooltip() {
		return _T("Add environment variable");
	}

	@Override
	protected DrawCardBeanItemEditPanel<EnvVar> newEditPanel(String id, List<EnvVar> items, int index,
			DrawCardBeanItemEditPanel.EditCallback callback) {
		return new EnvVarEditPanel(id, items, index, callback);
	}
