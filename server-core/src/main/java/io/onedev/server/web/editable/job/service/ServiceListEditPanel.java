...
class ServiceListEditPanel extends PropertyEditor<List<Serializable>> {

	private final List<JobService> services;
	
	public ServiceListEditPanel(String id, PropertyDescriptor propertyDescriptor, IModel<List<Serializable>> model) {
	...
	protected void onInitialize() {
		...
		add(new ModalLink("addNew") {
			...
			protected Component newContent(String id, ModalPanel modal) {
				return new ServiceEditPanel(id, services, -1) {
					...
					protected void onSave(AjaxRequestTarget target) {
						...
						target.add(ServiceListEditPanel.this);
					}

					@Override
					public BuildSpec getBuildSpec() {
						return ServiceListEditPanel.this.getBuildSpec();
					}

					@Override
					public Job getJob() {
						return ServiceListEditPanel.this.getJob();
					}
		...
		columns.add(new AbstractColumn<JobService, Void>(Model.of("")) {
			...
			@Override
			public void populateItem(Item<ICellPopulator<JobService>> cellItem, String componentId, IModel<JobService> rowModel) {
				Fragment fragment = new Fragment(componentId, "actionColumnFrag", ServiceListEditPanel.this);
				fragment.add(new ModalLink("edit") {

					@Override
					protected Component newContent(String id, ModalPanel modal) {
						return new ServiceEditPanel(id, services, cellItem.findParent(Item.class).getIndex()) {
