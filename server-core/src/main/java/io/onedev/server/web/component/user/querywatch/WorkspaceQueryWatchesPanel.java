...
class WorkspaceQueryWatchesPanel extends GenericPanel<User> {
    ...
    private final IModel<List<QueryInfo>> queryInfosModel = new LoadableDetachableModel<>() {
        ...
        protected List<QueryInfo> load() {
            ...
            var workspaceSetting = getSettingService().getWorkspaceSetting();
            for (var name : getUser().getWorkspaceQuerySubscriptions()) {
                NamedQuery query;
                if (name.startsWith(COMMON_NAME_PREFIX))
                    query = workspaceSetting.getNamedQuery(name.substring(COMMON_NAME_PREFIX.length()));
                else
                    query = getUser().getWorkspaceQuery(name.substring(PERSONAL_NAME_PREFIX.length()));

                if (query != null)
                    queryInfos.add(new QueryInfo(name, query.getQuery(), null));
            }
            for (var personalization : getUser().getWorkspaceQueryPersonalizations()) {
                for (var name : personalization.getQuerySubscriptions()) {
                    NamedQuery query;
                    if (name.startsWith(COMMON_NAME_PREFIX))
                        query = personalization.getProject().getNamedWorkspaceQuery(name.substring(COMMON_NAME_PREFIX.length()));
                    else
    ...
    public WorkspaceQueryWatchesPanel(String id, IModel<User> model) {
        super(id, model);
    }
                ...
                OneDev.getInstance(TransactionService.class).run(() -> {
                    ...
                    for (IModel<QueryInfo> each : selectionColumn.getSelections()) {
                        ...
                        if (queryInfo.projectId == null) {
                            getUser().getWorkspaceQuerySubscriptions().remove(queryInfo.name);
                            if (getPage() instanceof UserPage)
                                auditService.audit(null, "unwatched workspace query \"" + queryInfo.name + "\" in account \"" + getUser().getName() + "\"", null, null);
                        } else {
                            for (var personalization : getUser().getWorkspaceQueryPersonalizations()) {
                                if (personalization.getProject().getId().equals(queryInfo.projectId)) {
                                    ...
                                    getWorkspaceQueryPersonalizationService().createOrUpdate(personalization);
                                    if (getPage() instanceof UserPage)
                                        auditService.audit(null, "unwatched workspace query \"" + queryInfo.name + "\" in account \"" + getUser().getName() + "\" in project \"" + personalization.getProject().getPath() + "\"", null, null);
                                }
                            }
                        }
                    }
        ...
        columns.add(new AbstractColumn<>(Model.of("Name")) {
            ...
            public void populateItem(Item<ICellPopulator<QueryInfo>> cellItem, String componentId,
                ...
                if (projectId != null) {
                    ...
                    var link = new BookmarkablePageLink<Void>("link", ProjectWorkspacesPage.class, ProjectWorkspacesPage.paramsOf(project, rowModel.getObject().query, 1));
                    link.add(new Label("label", name));
                    fragment.add(link);
                } else {
                    var link = new BookmarkablePageLink<Void>("link", WorkspaceListPage.class, WorkspaceListPage.paramsOf(rowModel.getObject().query, 1));
                    link.add(new Label("label", name));
                    fragment.add(link);
                }
