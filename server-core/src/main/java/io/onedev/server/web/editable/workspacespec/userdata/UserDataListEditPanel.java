...
class UserDataListEditPanel extends PropertyEditor<List<Serializable>> {

	private final List<UserData> configs;
	
	public UserDataListEditPanel(String id, PropertyDescriptor propertyDescriptor, IModel<List<Serializable>> model) {
		super(id, propertyDescriptor, model);
		
		configs = new ArrayList<>();
		for (Serializable each: model.getObject()) {
			configs.add((UserData) each);
		}
	}
	...
	protected void onInitialize() {
		...
		add(new ModalLink("addNew") {
			...
			protected Component newContent(String id, ModalPanel modal) {
				return new UserDataEditPanel(id, configs, -1) {
					...
					protected void onSave(AjaxRequestTarget target) {
						...
						target.add(UserDataListEditPanel.this);
					}
		...
		columns.add(new AbstractColumn<UserData, Void>(Model.of("")) {
		...
		columns.add(new AbstractColumn<UserData, Void>(Model.of("")) {
			...
			@Override
			public void populateItem(Item<ICellPopulator<UserData>> cellItem, String componentId, IModel<UserData> rowModel) {
				Fragment fragment = new Fragment(componentId, "actionColumnFrag", UserDataListEditPanel.this);
				fragment.add(new ModalLink("edit") {

					@Override
					protected Component newContent(String id, ModalPanel modal) {
						return new UserDataEditPanel(id, configs, cellItem.findParent(Item.class).getIndex()) {
		...
		IDataProvider<UserData> dataProvider = new ListDataProvider<UserData>() {
		...
		DataTable<UserData, Void> dataTable;
		add(dataTable = new DataTable<UserData, Void>("configs", columns, dataProvider, Integer.MAX_VALUE));
