...
public class PullRequestNotificationManager extends AbstractNotificationManager {
	...
	public void on(PullRequestEvent event) {
		Long requestId = event.getRequest().getId();
						...
						LockUtils.call(PullRequest.getSerialLockName(requestId), true, new Callable<Void>() {
							...
							public Void call() throws Exception {
								transactionManager.run(new Runnable() {
									...
									@Override
									public void run() {
										PullRequestEvent clone = event.cloneIn(dao);
										PullRequest request = clone.getRequest();
										User user = clone.getUser();

										String url;
										if (clone instanceof PullRequestCommented)
											url = urlManager.urlFor(((PullRequestCommented)clone).getComment());
										else if (clone instanceof PullRequestChanged) 
											url = urlManager.urlFor(((PullRequestChanged)clone).getChange());
										else if (clone instanceof PullRequestCodeCommentCreated)
											url = urlManager.urlFor(((PullRequestCodeCommentCreated)clone).getComment());
										else if (clone instanceof PullRequestCodeCommentReplied)
											url = urlManager.urlFor(((PullRequestCodeCommentReplied)clone).getReply());
										else if (clone instanceof PullRequestCodeCommentStatusChanged)
											url = urlManager.urlFor(((PullRequestCodeCommentStatusChanged)clone).getChange());
										else 
