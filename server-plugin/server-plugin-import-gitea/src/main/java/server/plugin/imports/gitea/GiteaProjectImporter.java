...
public class GiteaProjectImporter extends ProjectImporter2<ImportServer, ProjectImportSource, ProjectImportOption> {
	...
	public ProjectImportSource getWhat(ImportServer where, SimpleLogger logger) {
		...
		Client client = ImportUtils.newClient(where);
		try {
